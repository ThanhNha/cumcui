...
function flatsome_follow( $atts, $content = null, $tag = '' ) {
	...
	if ( $use_global_link ) {
		...
		$behance   = get_theme_mod( 'follow_behance' );
	...
	$follow_links = apply_filters( 'flatsome_follow_links', array(
		...
		'behance'   => array(
			'enabled'  => ! empty( $behance ),
			'atts'     => array(
				'href'       => $behance,
				'data-label' => 'Behance',
				'target'     => '_blank',
				'rel'        => 'noopener noreferrer nofollow',
				'class'      => $style . ' behance tooltip',
				'title'      => esc_attr__( 'Follow on Behance', 'flatsome' ),
				'aria-label' => esc_attr__( 'Follow on Behance', 'flatsome' ),
			),
			'icon'     => get_flatsome_icon( 'icon-dribbble' ),
			'priority' => 50,
		),
		'email'     => array(
			'enabled'  => ! empty( $email ),
